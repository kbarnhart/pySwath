...
inPoly='/test/kennicottCenterline.shp' # input shapefile filename along which raster will be sampled
...
for lineNo in range(numLines): # need to add additional info for output files if there are more than one line. 

	lineX=[]
	lineY=[]
 	
 	feat=layer.GetFeature(lineNo)
	geom=feat.geometry()
	
	numPoints=geom.GetPointCount()
	
	for nP in range(numPoints):
		lineX.append(geom.GetPoint(nP)[0])
		lineY.append(geom.GetPoint(nP)[1])
		
	# interpolate to the spacing specified by height
